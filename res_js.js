...
/*ADS SORTING JS CODE: END*/
    ...
    document.getElementById('categoryChips').addEventListener('click',e=>{
      if(e.target.classList.contains('chip-cat-loader')) renderListings({category:e.target.dataset.cat})
    });

    // request trigger actions
    document.getElementById('heroCard').addEventListener('click',e=>{
      const CATEGORY = ['find lands for sale', 'find apartments', 'find projects'],
      ...
      localStorage.setItem('RefererCategory', `realestate_${cate_target}`);
      }
    });
