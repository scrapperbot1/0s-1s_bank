...
/*CATEGORY ADVERTS DISPLAY: START*/
    ...
    const properties_sales_and_rentals = [
      // Lands
      {id:1,category:'Standard Rooms',postedAt: '2025-09-18T08:30:00Z',pItems:'Chamber and Hall for rent - Tandra Hills',region:'Tandra Hill, Greater Accra Region',price:1800,unit:'GHS',img:'http://localhost/LandingPageRecs/PropetiesRentalsSectionRecs/photos/chamberandhalltantrahill.jpg',desc:'Chamber and hall self contained apt for rent at Tantra Hills.'},
      {id:2,category:'Apartments',postedAt: '2025-08-22T14:00:00Z',pItems:'Furnished 1bdrm Apartment in Eaglet Real Estate, East Legon for rent',region:'East Legon',price:3500,unit:'GHS',img:'http://localhost/LandingPageRecs/PropetiesRentalsSectionRecs/photos/492946821.jpg',desc:'Fully furnished one bed room studio apartment for rent at east legon hills , very nice environment'},
      // Apartments
      {id:3,category:'Standard Rooms',postedAt: '2025-09-25T09:15:00Z',pItems:'Single room for rent at Elmina',region:'Elmina',price:1500,unit:'GHS',img:'http://localhost/LandingPageRecs/PropetiesRentalsSectionRecs/photos/222.jpeg',desc:'Executive walled and gated Single room self contained going at Teshie Bushroad going for Gh1,500 a month for one year. *Service charges included*'},
      {id:4,category:'Items for sale',postedAt: '2025-09-10T12:00:00Z',pItems:'Bedside Table Top Fridge With Key 131',region:'Koforidua',price:1700,unit:'GHS',img:'http://localhost/LandingPageRecs/PropetiesRentalsSectionRecs/photos/44288542_MTExMi0xNjAwLWIyYzcwNjI1ZWM.webp',desc:'Quality and affordable, one year warranty'},
      // Projects
      {id:5,category:'Apartments',postedAt: '2025-07-01T10:00:00Z',pItems:'Furnished 2bdrm Apartment in Villaggio, Accra Metropolitan for Rent',region:'Greater Accra',price:39475,unit:'GHS',img:'http://localhost/LandingPageRecs/PropetiesRentalsSectionRecs/photos/Apartment_Villagio_Ghana.jpg',desc:'An amazing 2 bedroom apartment with high quality furnishings and equipment at airport villaggio.'},
      {id:6,category:'Items for Hire',postedAt: '2025-06-15T16:45:00Z',pItems:'Scafold for hire at oti region',region:'oti region',price:200,unit:'GHS',img:'http://localhost/LandingPageRecs/PropetiesRentalsSectionRecs/photos/FB_IMG_1623644847318.jpg',desc:'Want to find the best scaffolds rental in Ghana? Want to know the price of scaffolds rental in Ghana? You can get all the product and shop information you desire right here on reapp.com.gh, your product information portal. Always choose to shop the easy and convenient way!'}
    ];
//Standard Rooms for rent
    // DOM refs
    ...
    // render listings
    function renderListings(properties_sales_and_rentals_arg2=properties_sales_and_rentals,filters={}){
      let out = [...properties_sales_and_rentals_arg2];
      // category
      ...
      // search
      if(filters.q) out = out.filter(p=> (p.pItems+" "+p.region+" "+p.desc).toLowerCase().includes(filters.q.toLowerCase()));
      // sort
      ...
      listingsEl.innerHTML = out.map((p, p_index)=>`<div class="card" data-postid="${p_index+1}">
        ...
        <div style="margin-top:10px;display:flex;flex-direction:column;">
          <div class="title">${p.pItems}</div><small style="color:#777;"> 👁️: 25k</small>
...
document.addEventListener('DOMContentLoaded', () => {
  renderListings(sortProperties(properties_sales_and_rentals, 'new'));
  sortSelect.addEventListener('change', function () {
    const sorted = sortProperties(properties_sales_and_rentals, this.value);
...
/*ADS SORTING JS CODE: END*/
    ...
    document.getElementById('categoryChips').addEventListener('click',e=>{
      if(e.target.classList.contains('chip-cat-loader')) renderListings(properties_sales_and_rentals,{category:e.target.dataset.cat})
    });


    // request trigger actions
    document.getElementById('heroCard3').addEventListener('click',e=>{
      const CATEGORY = ['find rooms', 'find apartments', 'find items for hire', 'find items for sale'],
      ...
      localStorage.setItem('RefererCategory', `propertiessalesandrentals_${cate_target}`);
      }
    });
